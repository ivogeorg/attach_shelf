...
CartApproach::CartApproach()
    : Node("tf_relative_to_laser_test_node"),
      cb_group_{
...
void CartApproach::init_broadcaster_tfs() {
  RCLCPP_DEBUG(this->get_logger(), "Initializing relative TFs");

  tf_vector.push_back(std::make_pair("tf_1", std::make_tuple(1.0, 1.0, 0.0)));
  tf_vector.push_back(std::make_pair("tf_2", std::make_tuple(1.0, -1.0, 0.0)));
  tf_vector.push_back(
      std::make_pair("tf_3", std::make_tuple(2.0, 1.0, PI_ / 2.0)));
  tf_vector.push_back(
      std::make_pair("tf_4", std::make_tuple(2.0, -1.0, -PI_ / 2.0)));
}

void CartApproach::broadcaster_cb() {

  geometry_msgs::msg::TransformStamped ts_msg;
  for (auto &tf : tf_vector) {
    double x, y, yaw;
